    ...
    if (Array.isArray(apiResponse)) {
        ...
        await Promise.all(
            apiResponse.map(async (bp) => {
                ...
                const selectQry = SELECT.from('com_teched_event_A_BusinessPartner').columns('*').where('BusinessPartner=', bpID);
                let entries;
                try {
                ...
                if (entries.length === 0) {
                    ...
                    const insertBPStmt = INSERT.into('com_teched_event_A_BusinessPartner')
                    ...
                    if (bp.to_BusinessPartnerAddress && Array.isArray(bp.to_BusinessPartnerAddress)) {
                        ...
                        await Promise.all(
                            bp.to_BusinessPartnerAddress.map(async (address) => {
                                ...
                                const insertAddressStmt = INSERT.into('com_teched_event_A_BusinessPartnerAddress')
                    ...
                    if (entries[0].VERIFICATIONSTATUS_CODE !== 'P' && entries[0].VERIFICATIONSTATUS_CODE !== 'INV') {
                        ...
                        const updateBPStmt = UPDATE('com_teched_event_A_BusinessPartner')
                            .with({
                        ...
                        if (bp.to_BusinessPartnerAddress && Array.isArray(bp.to_BusinessPartnerAddress)) {
                            ...
                            await Promise.all(
                                ...
                                bp.to_BusinessPartnerAddress.map(async (address) => {
                                    ...
                                    const updateAddressStmt = UPDATE('com_teched_event_A_BusinessPartnerAddress')
                                        .with({
...
async function syncData(req) {
    ...
    if (verificationStatus_code !== null && verificationStatus_code.length > 0) {
        ...
        const selectBPStmt = SELECT.from('com_teched_event_A_BusinessPartner')
            .columns('*')
            .where('BusinessPartner =', bpID);
        const bpEntry = await db.run(selectBPStmt);

        const selectAddressStmt = SELECT.from('com_teched_event_A_BusinessPartnerAddress')
            .columns('*')
            .where('BusinessPartner_BusinessPartner =', bpID);
