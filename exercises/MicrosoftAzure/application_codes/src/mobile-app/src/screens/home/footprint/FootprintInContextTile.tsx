...
const FootprintInContextTile = () => {
    ...
    const indiaAverage = 1890;
    const apjAverage = 4908;
    ...
    const apjStepsPercent = Math.trunc((currentYearForecast / apjAverage) * 100);
    const apjSteps = Math.min(apjStepsPercent, 100);
    const indiaStepsPercent = Math.trunc((currentYearForecast / indiaAverage) * 100);
    const indiaSteps = Math.min(indiaStepsPercent, 100);
                ...
                <View style={{ marginBottom: 8 }}>
                    <View style={{ marginBottom: 16 }}>
                        <FootprintProgressBar
                            ...
                            text1="India"
                            text2={`${indiaAverage} kg`}
                            percentage={`${indiaStepsPercent}%`}
                            height={22}
                        />
                    </View>
                    <View style={{ marginBottom: 16 }}>
                        <FootprintProgressBar
                            step={apjSteps}
                            steps={100}
                            source={require("../../../assets/apjFlag.png")}
                            text1="APJ"
                            text2={`${apjAverage} kg`}
                            percentage={`${apjStepsPercent}%`}
                            height={22}
                        />
                    </View>
                    <View>
