...
export class ConverterService extends ApplicationService {
    ...
    /**
     * Forwards prompt of the payload via a destination (mapped as SAPGENAILLMACCESSSERVICE) through an SAP AI Core deployed service to Azure OpenAI services
     *
     * @param {string} prompt
    ...
    private callSAPGENAILLMACCESSSERVICE = async (prompt: string) => {
        try {
            const llmAccessService = await cds.connect.to("SAPGENAILLMACCESSSERVICE");
            const payload = {
                ...LLM_PARAMS,
...
---+--- 
        ...
        const response = await this.callSAPGENAILLMACCESSSERVICE(prompt);
    ...
    private generateSuggestions = async (req: Request): Promise<cs.ActionGenerateSuggestionsReturn> => {
        try {
            ...
            const prompt = `Provide 5 suggestions with a title and description (2-3 sentences) as a JSON array in the format [{"title": "", "description": "" }] without any additional text to help me reduce my CO2 footprint of ${CO2Score.toFixed(
                ...
                0
            )} INR with ${amountOfTransactions} transactions. Act as a REST API with Content-Type application/json,keep only json array in response`;
